...
- SPI (v2, as on the W5): a master clocks only with its SCK pin in the
  alternate function (the serial side runs from SCK, RM0481 §52.4.1); a pin
  left analog gives a bounded `HAL_TIMEOUT`. coregen sets the pins. The SPI
  kernel clock is per_ck = HSI (64 MHz), not SYSCLK. DMA: GPDMA1 CH6/CH7.
- core_nvm lives in the flash high-cycle data area (EDATA). `core_init()`
  calls `_core_nvm_h5_boot()` right after the clocks: on a Core's first boot
  it switches the area on (option byte FLASH_EDATA2R, nothing else) and
  resets once; every later boot only reads the option.
