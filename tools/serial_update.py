...
def query(link: Link, timeout: float = 1.5) -> dict:
    ...
    while time.monotonic() < deadline:
        ...
        if r and r[1] == "READY" and len(r) >= 7:
            return {"version": int(r[2]), "dev_id": int(r[3], 16), "flash": int(r[4]) * 1024,
                    "page": int(r[5]), "page0_erased": r[6] == "1"}
...
def main() -> int:
    ...
    ap.add_argument("--dev-id", type=lambda s: int(s, 0), default=DEV_ID_L41X_L42X,
                    help="expected DEV_ID (default 0x464, STM32L41x/L42x)")
    args = ap.parse_args()
    ...
    try:
        ...
        with serial.Serial(port, TRIGGER_BAUD, timeout=0.05) as ser:
            update(SerialLink(ser), image, args.dev_id,
