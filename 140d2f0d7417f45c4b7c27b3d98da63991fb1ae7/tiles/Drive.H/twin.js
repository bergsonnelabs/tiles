...
var INIT_CONFIG = {
  ...
  closed_loop: 1,
  rated_voltage: 86,
  // ~2.2 Vrms at 238 Hz / 300 µs (SLOS825E Eq 3)
  od_clamp: 140,
  // 3.07 V peak clamp (Eq 7)
  fb_brake: 3,
  // 0xB6[6:4]
  loop_gain: 1,
  // 0xB6[3:2]
...
function fullScaleMv(s) {
  if (!s.n_erm_lra) return s.closed_loop ? s.rated_voltage * 21.33 : s.od_clamp * 21.96;
  const f = 1e6 / (2 * (500 + 100 * (s.drive_time & 31)));
  const ts = [150, 200, 250, 300][s.sample_time & 3] * 1e-6;
  if (s.closed_loop)
    return s.rated_voltage * 20.71 / Math.sqrt(Math.max(0.1, 1 - (4 * ts + 3e-4) * f));
  return s.od_clamp * 21.32 * Math.sqrt(Math.max(0.1, 1 - f * 8e-4));
}
function isqrt(x) {
...
var sim = {
  ...
  hostCalls: {
    ...
    tile_drive_h_set_audio_envelope: ({ state, args }) => ready(state) ? { nextState: { atv_peak_time: (args[0] ?? 0) & 3, atv_filter: (args[1] ?? 0) & 3 } } : {},
    // ATV 0x12-0x15 written as given, no sentinel.
    tile_drive_h_set_audio_levels: ({ state, args }) => ready(state) ? {
      nextState: {
        atv_min_input: (args[0] ?? 0) & 255,
        atv_max_input: (args[1] ?? 0) & 255,
        atv_min_drive: (args[2] ?? 0) & 255,
        atv_max_drive: (args[3] ?? 0) & 255
      }
    } : {},
  ...
  provenance: {
    ...
    tile_drive_h_set_brake_factor: "canonical",
    // FEEDBACK_CTRL[6:4] (stored; waveform effect not modeled)
    tile_drive_h_set_loop_gain: "canonical",
    // FEEDBACK_CTRL[3:2] (stored; waveform effect not modeled)
    ...
    power: "inferred"
    // chip currents and Eq 2-5 from the datasheet; actuator load an assumed 25 Ω LRA
  },
