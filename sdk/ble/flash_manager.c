...
/* Includes ------------------------------------------------------------------*/
/* tiles: UTILS_ENTER_CRITICAL_SECTION declares primask_bit where it is used,
 * and this file nests it; the shadowing is harmless (each EXIT restores its
 * own ENTER). */
...
/* tiles: this SDK has no ST HAL. The flash unlock / lock come from ll_flash.h,
 * and FLASH_BASE there is the flash *register* block, so the memory base and
 * size are FLASH_START / FLASH_TOTAL_SIZE. Other tiles changes are marked. */
#include "ll_flash.h"
#define HAL_FLASH_Unlock()  ll_flash_unlock()
#define HAL_FLASH_Lock()    ll_flash_lock()
#define FM_FLASH_BASE       FLASH_START
#define FM_FLASH_SIZE       FLASH_TOTAL_SIZE
...
/* Private defines -----------------------------------------------------------*/
#define FLASH_PAGE_NBR    (FM_FLASH_SIZE / FLASH_PAGE_SIZE)
...
FM_Cmd_Status_t FM_Write(uint32_t *Src, uint32_t *Dest, int32_t Size, FM_CallbackNode_t *CallbackNode)
{
  ...
  if (((uint32_t)Dest < FM_FLASH_BASE) || ((uint32_t)Dest > (FM_FLASH_BASE + FM_FLASH_SIZE))
                                    || (((uint32_t)Dest + Size) > (FM_FLASH_BASE + FM_FLASH_SIZE)))
  {
...
void FM_BackgroundProcess (void)
{
  ...
  /* tiles: a flash hardware error (FLASH_NSSR) ends the operation. Upstream,
   * FD_* report it the same way as a closed time window, so the manager would
   * retry the failed quad-word in every new window, forever. The requester
   * reads the error with FD_TakeHwError(). */
  if (FD_HwErrorPending())
  {
    flashop_complete = true;
  }
