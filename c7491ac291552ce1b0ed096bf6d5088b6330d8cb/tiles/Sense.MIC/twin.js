...
var ADC_MAX = 4095;
var AMP_GAIN = 48;
var MIC_MV_PER_PA = 7.943;
var RAIL_MV = 3300;
var BIAS_MV = RAIL_MV / 2;
var SAMPLES_PER_CYCLE = 16;
var TIER2_BUF_LEN = 64;
var AMP_GAIN_DX_DB = 336;
var LOG10_X20 = [
  0,
  0,
  60,
...
var refMv = (ref) => ref === 5 || ref === 7 ? 2048 : 3300;
function peakMv(s) {
...
var sim = {
  ...
  controls: [
    {
      ...
      step: 10,
      unit: "0.1 dB SPL",
      description: "Ambient sound in 0.1 dB (700 = 70 dB). The driver's SPL readout saturates near 72.5 dB."
    },
    {
      type: "toggle",
      field: "clap_event",
  ...
  hostCalls: {
    ...
    tile_sense_mic_detect_clap: ({ state }) => state.clap_event ? { scalar: 1, nextState: { clap_event: 0 } } : { scalar: 0 }
  },
  provenance: {
    ...
    power: "inferred"
    // datasheet typicals summed: mic 250 µA, AD8605 1 mA, ADC per reference
  },
  ...
  power(state) {
    const refOn = state.ref_sel === 5 || state.ref_sel === 7;
    const adc = refOn ? 330 : 0.5;
    const ua = Math.round(250 + 1e3 + adc);
    return {
      draw_ua: ua,
      rails: [
        {
