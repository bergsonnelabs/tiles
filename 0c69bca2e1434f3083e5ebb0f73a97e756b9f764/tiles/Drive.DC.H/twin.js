...
var W_SCALE = [16, 32, 64, 128];
var TWO_PI_X1000 = 6283;
var rpmToRad = (rpm, rpr) => rpm * rpr * 2 * Math.PI / 60;
var radToRpm = (rad, rpr) => rad * 60 / (2 * Math.PI * Math.max(1, rpr));
var COAST = 0;
var FORWARD = 1;
var REVERSE = 2;
var BRAKE = 3;
...
function motorOp(s) {
  ...
  const rpmPerMv = 1e3 / Math.max(1, s.motor_kv) * (1 - L);
  let v;
...
var speedRaw = (s, op) => s.rc_enabled && op.rpm > 0 ? clamp(Math.round(rpmToRad(op.rpm, s.ripples_per_rev) / s.w_scale), 0, 255) : 0;
var vmtrMv = (s, op) => {
...
var sim = {
  ...
  controls: [
    ...
    },
    {
      ...
      step: 10,
      unit: "\xB5V/RPM",
      description: "Back-EMF constant of the attached motor: sets speed per volt. A placeholder until the program's set_motor_params() gives the real value."
    },
    {
      type: "slider",
      field: "load",
  ...
  hostCalls: {
    ...
    tile_drive_dc_h_get_speed_rpm: ({ state }) => {
      ...
      return {
        scalar: Math.floor(
          raw * state.w_scale * 6e4 / (TWO_PI_X1000 * Math.max(1, state.ripples_per_rev))
        )
      };
    },
    ...
    tile_drive_dc_h_set_speed_rpm: ({ state, args }) => {
      if (!ready(state)) return {};
      const rpm = Math.min(4e5, Math.max(0, num(args, 0, 0)));
      const rpr = Math.max(1, state.ripples_per_rev);
      const rpmRpr = Math.min(4e5, rpm * rpr);
      let code = 3;
      let wset = 255;
      for (let k = 0; k < 4; k++) {
        const den = 6e4 * W_SCALE[k];
