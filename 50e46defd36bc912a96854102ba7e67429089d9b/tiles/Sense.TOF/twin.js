...
function rawReliability(s) {
  return rawDistance(s) > 0 ? s.reliability & 63 : 0;
}
function status(s) {
...
var sim = {
  ...
  hostCalls: {
    ...
    tile_sense_tof_set_distance_mode: ({ state, args }) => ({
      nextState: { distance_mode: arg(args, 0, state.distance_mode) & 255 }
    }),
  ...
  provenance: {
    ...
    // driver: reliability ≥ 32, distance > 0, ≤ mm
    ...
    tile_sense_tof_get_sys_clock_ticks: "inferred",
