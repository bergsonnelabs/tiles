...
var LDO_LIMIT_UA = 1e5;
var GND_SWITCH_VTH_MV = 1e3;
var TS_STRAP_MV = 400;
var TS_HOT_MV = 185;
var TS_COLD_MV = 585;
var CS_IDLE = 0;
...
function chargeState(s, i) {
  if (!vinPgood(i) || !batteryPresent(i) || !s.charging_enabled || s.pmid_mode !== PMID_AUTO || // BAT_ONLY stops charging; FLOAT / PULLDOWN cut PMID
  tsFault(s))
    return CS_IDLE;
...
function driverCharging(s, i) {
  const s0 = stat0(s, i);
  const s1 = stat1(s, i);
  return (s0 & 1) !== 0 && (s0 & 32) === 0 && (s1 & 153) === 0 && s.charging_enabled === 1 && s.pmid_mode === PMID_AUTO;
}
...
var sim = {
  ...
  provenance: {
    ...
    tile_power_l_1t_get_ts_mv: "canonical",
    // 5 kΩ strap × 80 µA bias = 400 mV
    ...
    // 600 mV + 100 mV·code
    tile_power_l_1t_set_ldo_mode: "canonical",
    // VINLS tied to PMID (schematic)
    ...
    power: "inferred"
    // quiescents + LDO rating canonical; charge/LDO currents modeled
  },
