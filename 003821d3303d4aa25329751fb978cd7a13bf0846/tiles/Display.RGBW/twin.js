...
var sim = {
  ...
  provenance: {
    ...
    power: "inferred"
    // ISD / ISTB / INOR canonical; boost efficiency 90 % (datasheet guidance); EN pull-up from tile schematic
  },
  ...
  power(state, ctx) {
    ...
    if (vin < VIN_UVLO_MV) {
      ...
      draw = ISD_UA + Math.round(vin * 1e3 / EN_PULLUP_OHM);
      ...
      const over = CH.filter(
        (ch) => level(state, ch) > 0 && dcOf(state, ch) / 255 * fullScaleMa(state) > LED_IF_MAX_MA
      );
      note = over.length ? `boost + LED current; ${over.join("/").toUpperCase()} above the LED's 5 mA rating` : "boost + LED current";
    }
    const rails = [
