// Digital twin for Sense.I.9 — TDK InvenSense ICM-20948: a 6-axis IMU with an
// AK09916 magnetometer on the same package, reached through the ICM's I2C
// bypass (the driver sets INT_PIN_CFG.BYPASS_EN at init).
//
// ONE module serves the simulator worker (host calls) and the main thread
// (power / pads), on one field vocabulary.
//
// Behavior follows the driver (tiles/drivers/tile_sense_i_9.{h,c}) and the
// datasheet (DS-000189 rev 1.3):
//   - init: soft reset, wake (PWR_MGMT_1 = 0x01), all accel + gyro axes on,
//     INT_PIN_CFG = 0x02 (bypass; INT active-high, push-pull, 50 µs pulse), the
//     AK09916 in continuous 100 Hz. It never touches ACCEL_CONFIG /
//     GYRO_CONFIG_1, so they keep their reset value 0x01: ±2 g, ±250 dps, DLPF on,
//     1125 Hz (dividers 0).
//   - reset() (DEVICE_RESET) leaves PWR_MGMT_1 at its reset value 0x41 — the chip
//     ASLEEP — and INT_PIN_CFG at 0x00, which drops BYPASS_EN: the AK09916 stops
//     answering until init() runs again.
//   - INT_STATUS (WOM) and INT_STATUS_1 (RAW_DATA_RDY) clear on read.
//   - The face / motion helpers compare raw counts against constants fixed for
//     ±2 g (driver .c:666-676), so after `set_accel_range` they drift — the twin
//     does the same.
//
...
interface State {
  ...
  /** A faulty part: self-tests fail and the magnetometer reports overflow. */
  fault_inject: number;

  // ── configuration (the driver's setters) ──
  accel_range: number; // sense_i_9_accel_range_t: 0 ±2 g, 2 ±4, 4 ±8, 6 ±16
  gyro_range: number; // sense_i_9_gyro_range_t: 0 ±250, 2 ±500, 4 ±1000, 6 ±2000 dps
  mag_mode: number; // AK09916 CNTL2 (sense_i_9_mag_mode_t)
  accel_divider: number; // ACCEL_SMPLRT_DIV: ODR = 1125 / (1 + div)
  gyro_divider: number; // GYRO_SMPLRT_DIV: ODR = 1125 / (1 + div)
  /** PWR_MGMT_1.SLEEP. */
  sleeping: number;

  // ── INT pin ──
  /** INT_PIN_CFG: bit7 active-low, bit6 open-drain, bit5 latched, bit4 any-read
   * clear, bit1 BYPASS_EN. */
  int_pin_cfg: number;
  int_dry_en: number; // INT_ENABLE_1 bit0
  int_wom_en: number; // INT_ENABLE bit3
  int_fifo_ovf_en: number; // INT_ENABLE_2
  int_fifo_wm_en: number; // INT_ENABLE_3

  // ── latched status (clear on read) ──
  /** INT_STATUS: bit3 WOM_INT. */
  int_status: number;
  /** INT_STATUS_1: bit0 RAW_DATA_0_RDY_INT. */
  int_status_1: number;
  /** INT_STATUS_2 [4:0]: FIFO overflow (bit0 = FIFO 0). */
  int_status_fifo_ovf: number;
  /** INT_STATUS_3 [4:0]: FIFO watermark. */
  int_status_fifo_wm: number;
  /** Clock (deriveState `t`) of the last RAW_DATA_RDY. */
  last_drdy_ms: number;

  // ── wake-on-motion ──
  /** ACCEL_INTEL_CTRL.ACCEL_INTEL_EN. */
  wom_global_en: number;
  /** ACCEL_WOM_THR as the chip holds it (4 mg/LSB, max 1020), mg. */
  wom_threshold_mg: number;
  /** ACCEL_INTEL_CTRL bit0: 0 compare to the first sample, 1 to the previous. */
  wom_mode: number;
  /** The sample WOM compares against, mg. */
  wom_ref_x_mg: number;
  wom_ref_y_mg: number;
  wom_ref_z_mg: number;
  /** Per-axis edge memory: only a fresh crossing latches WOM_INT. */
  prev_wom_x: number;
  prev_wom_y: number;
  prev_wom_z: number;

  // ── FIFO ──
  fifo_mode: number; // 0 stream, 0x1F snapshot
  fifo_accel_en: number;
  fifo_gyro_en: number;
  fifo_temp_en: number;
  /** Bytes waiting in the FIFO (FIFO_COUNT). */
  fifo_bytes: number;
  /** Clock of the last sample pushed; -1 = restart on the next tick. */
  last_fifo_ms: number;
}
...
const INT_WOM = 1 << 3;
const RAW_DATA_RDY = 1 << 0;
const FIFO0 = 1 << 0;
const BYPASS_EN = 0x02;
const FIFO_BYTES = 512;
...
const accelLsb = (s: State) => ACCEL_LSB_PER_G[s.accel_range & 0x06] ?? 16384;
const gyroLsb = (s: State) => GYRO_LSB_PER_DPS[s.gyro_range & 0x06] ?? 131;
...
const magReachable = (s: State) => (s.int_pin_cfg & BYPASS_EN) !== 0;
const magMeasuring = (s: State) => s.mag_mode !== 0x00 && !s.sleeping;
/** HOFL: |HX| + |HY| + |HZ| exceeds 4912 µT (AK09916 §8.3.5). */
const magOverflow = (s: State) =>
  Math.abs(s.mag_x_ut) + Math.abs(s.mag_y_ut) + Math.abs(s.mag_z_ut) > 4912;
...
const sim: TileSim<State> = {
  ...
  defaultState: {
    accel_x_mg: 0,
    accel_y_mg: 0,
    accel_z_mg: 1000, // resting flat, face up
    gyro_x_dps: 0,
    gyro_y_dps: 0,
    gyro_z_dps: 0,
    mag_x_ut: 25, // ~Earth's field, horizontal
    mag_y_ut: 0,
    mag_z_ut: -45, // ~Earth's field, vertical
    temperature_c: 22,
    fault_inject: 0,

    accel_range: 0, // ±2 g (ACCEL_CONFIG reset 0x01; init doesn't change it)
    gyro_range: 0, // ±250 dps (GYRO_CONFIG_1 reset 0x01)
    mag_mode: 0x08, // continuous 100 Hz (init)
    accel_divider: 0,
    gyro_divider: 0,
    sleeping: 0,

    int_pin_cfg: 0x02, // init: BYPASS_EN, active-high, push-pull, pulsed
    int_dry_en: 0,
    int_wom_en: 0,
    int_fifo_ovf_en: 0,
    int_fifo_wm_en: 0,
    ...
    wom_global_en: 0,
    wom_threshold_mg: 0, // ACCEL_WOM_THR reset 0x00
    wom_mode: 0,
    wom_ref_x_mg: 0,
    wom_ref_y_mg: 0,
    wom_ref_z_mg: 1000,
  ...
  hostCalls: {
    ...
    tile_sense_i_9_set_gyro_odr: ({ args }) => ({
      nextState: { gyro_divider: (args[0] ?? 0) & 0xff },
    }),
    ...
    tile_sense_i_9_self_test: ({ state }) => {
      const pass = state.fault_inject ? 0 : 0x07;
      return {
    ...
    tile_sense_i_9_mag_self_test: ({ state }) =>
      magReachable(state)
        ? { scalar: state.fault_inject ? 0 : 1, nextState: { mag_mode: 0x00 } }
        : { scalar: 0 },
    ...
    tile_sense_i_9_dmp_stop: () => undefined,
  ...
  deriveState(state, { t }) {
    ...
    // FIFO: accel/gyro/temp bytes at the sample rate; 512 bytes.
