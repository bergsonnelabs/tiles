// Digital twin for Display.RGBW — TI LP5811 4-channel LED driver + on-tile RGBW LED.
//
...
// V+ (10).
//
// ONE state, two readers: the firmware's calls (worker) and indicators / power
// (main thread) read the same fields, named as the driver's setters name them
// (`r`…`w` PWM, `r_curr`…`w_curr` DC current codes, `enabled` = CHIP_EN).
//
// Behavior follows tile_display_rgbw.c + the LP5811 datasheet (SNVSCC4C) and
// register map: init leaves CHIP_EN = 1, boost 4.5 V, MC = 51 mA, DC = 5 on every
// channel, Dev_Config_12 = lod_action 1 / lsd_action 0 / threshold 3; RESET puts
// every register back to 0 (Dev_Config_12 to 0x08) — dark until init runs again;
// a short with lsd_action set is OFAF (all LEDs off until Fault_Clear); pulse /
// breathe / flash are the driver's software helpers and end with the LEDs OFF.
// Those helpers block on hardware; here the call returns and the tick plays the
// same envelope over time.
import type { PowerCtx, PowerRail, TileIndicator, TileSim } from '../tileSim';

const DC_DEFAULT = 5; // LP5811_DC_DEFAULT: ~1 mA per channel at the 51 mA scale
const VOUT_INIT_MV = 4500; // init ramps the boost to 4.5 V
const VOUT_RESET_MV = 3000; // Dev_Config_0 reset: boost code 0 = 3.0 V
const BOOST_EFF = 0.9; // datasheet: "use 90 % for most applications"
const VIN_UVLO_MV = 1800; // VIN rising UVLO (max)
const EN_VIL_MV = 400; // EN read low below this
// Supply currents (datasheet §6.5)
...
const HELPER_PULSE = 1;
const HELPER_BREATHE = 2;
const HELPER_FLASH = 3;

interface State {
  // ── manual output (Manual_PWM / Manual_DC) ──
  r: number;
  g: number;
  b: number;
  w: number;
  ...
  b_curr: number;
  w_curr: number;
  enabled: number; // CHIP_EN
  max_current_mode: number; // MC: 0 = 25.5 mA, 1 = 51 mA full scale
  boost_mv: number; // boost_vout

  // ── faults + protection config (Dev_Config_12, status pages) ──
  lsd_threshold: number;
  lsd_shutdown: number; // lsd_action: a short → OFAF
  lod_shutdown: number; // lod_action: an open shuts that sink
  fault_inject: number; // stimulus: a shorted LED on every channel
  fault_short_mask: number; // LSD_STATUS_0[3:0]
  fault_open_mask: number; // LOD_STATUS_0[3:0]
  fault_tsd: number;
  fault_config: number;

  // ── driver software helpers (pulse / breathe / flash) ──
  helper_running: number;
  helper_kind: number;
  helper_r: number; // the helper's peak colour
  helper_g: number;
  helper_b: number;
  helper_ms: number; // pulse length
  breathe_period_ms: number;
  flash_count: number;
  helper_start_ms: number; // tick clock when it began; -1 = not yet stamped

  // ── autonomous engine (AEU) ──
  auto_r: number; // Dev_Config_3 per-LED autonomous enable
  auto_g: number;
  auto_b: number;
  auto_w: number;
  anim_channel: number; // LED index the modeled breathe drives
  anim_peak: number;
  anim_period_ms: number;
  animating: number; // Start_cmd received
  anim_paused: number;
  anim_clock: number; // ms the engine has run
  exp_mask: number; // Dev_Config_5
  phase_align: number;
  aeu_configured: number;

  // ── supply stimulus (standalone) + tick clock ──
  vplus_mv: number;
  last_tick_ms: number;
}
...
const u8 = (args: number[], i: number) => num(args, i, 0) & 0xff;
const clamp = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));

const fullScaleMa = (s: State) => (s.max_current_mode === 1 ? 51 : 25.5);
const autoOn = (s: State, ch: Ch) =>
  (ch === 'r' ? s.auto_r : ch === 'g' ? s.auto_g : ch === 'b' ? s.auto_b : s.auto_w) === 1;
const pwmOf = (s: State, ch: Ch) => (ch === 'r' ? s.r : ch === 'g' ? s.g : ch === 'b' ? s.b : s.w);
const dcOf = (s: State, ch: Ch) =>
  ch === 'r' ? s.r_curr : ch === 'g' ? s.g_curr : ch === 'b' ? s.b_curr : s.w_curr;
const bitOf = (ch: Ch) => 1 << CH.indexOf(ch);

// Autonomous breathe: 0 → peak → 0 over the period (the AEU's 5 keyframes).
...
// OFAF: a latched short with lsd_action set turns every LED off.
const ofaf = (s: State) => s.lsd_shutdown === 1 && s.fault_short_mask !== 0;

// Light out of one channel, 0…1 of the sink's full scale.
function level(s: State, ch: Ch): number {
  if (s.enabled !== 1 || ofaf(s)) return 0;
  if (s.fault_open_mask & bitOf(ch)) return 0; // open LED: no light either way
  return clamp(duty(s, ch) * (dcOf(s, ch) / 255), 0, 1);
}

// Supplies as the chip sees them: the wired nets when solved, else the stimulus.
function supplies(s: State, ctx?: PowerCtx): { vin: number; enHigh: boolean } {
...
const chipAlive = (s: State, ctx?: PowerCtx) => {
  const { vin, enHigh } = supplies(s, ctx);
  return vin >= VIN_UVLO_MV && enHigh;
};

const cancelHelper = { helper_running: 0, helper_kind: 0, helper_start_ms: -1 } as const;
const startHelper = (kind: number, args: number[]) => ({
  helper_running: 1,
  helper_kind: kind,
  helper_r: u8(args, 0),
  helper_g: u8(args, 1),
...
// Registers after RESET (0x66): everything 0, Dev_Config_12 = 0x08.
...
const sim: TileSim<State> = {
  ...
  controls: [
    {
      ...
      description:
        'A short on every channel: latches the LSD status (is_faulted → 1) and, after set_short_shutdown(1), trips OFAF — all LEDs off until clear_faults.',
    },
  ],
  ...
  hostCalls: {
    ...
    tile_display_rgbw_breathe: ({ args }) => ({
      nextState: {
        ...
        breathe_period_ms: num(args, 3, 0) & 0xffff,
        r: 0,
        g: 0,
        b: 0,
        w: 0,
    ...
    tile_display_rgbw_is_faulted: ({ state }) => ({
      scalar:
        state.fault_short_mask ||
        state.fault_open_mask ||
        state.fault_tsd ||
        state.fault_config ||
        state.fault_inject
          ? 1
    ...
    tile_display_rgbw_set_short_threshold: ({ args }) => ({
      nextState: { lsd_threshold: num(args, 0, 3) & 0x03 },
    }),
    ...
    tile_display_rgbw_set_phase_align: ({ state, args }) => {
      ...
      return {
        nextState: {
          phase_align: (state.phase_align & ~(0x03 << shift)) | ((num(args, 1, 0) & 0x03) << shift),
        },
      };
    },
    ...
    tile_display_rgbw_breathe_auto: ({ state, args }) => {
      ...
      return {
        nextState: {
          anim_channel: ch,
          anim_peak: u8(args, 1),
          anim_period_ms: num(args, 2, 2000) & 0xffff,
          aeu_configured: 1,
  ...
  provenance: {
    ...
    tile_display_rgbw_init: 'canonical', // defaultState = post-init registers
    tile_display_rgbw_sleep: 'canonical', // CHIP_EN = 0
    tile_display_rgbw_wake: 'canonical',
    tile_display_rgbw_reset: 'canonical', // register reset values
    ...
    tile_display_rgbw_pulse: 'inferred', // driver sequence; timing played by the tick
    tile_display_rgbw_breathe: 'inferred', // 32-step ramp modeled as a triangle
    tile_display_rgbw_flash: 'inferred', // 100 ms on / 100 ms off
    tile_display_rgbw_read_faults: 'inferred', // struct out not readable from the DSL
    tile_display_rgbw_clear_faults: 'canonical',
    tile_display_rgbw_is_faulted: 'canonical',
    tile_display_rgbw_set_short_threshold: 'inferred', // stored; the injected short trips at any threshold
    tile_display_rgbw_set_short_shutdown: 'canonical', // lsd_action → OFAF
    tile_display_rgbw_set_open_shutdown: 'inferred', // stored; no open-LED stimulus
    tile_display_rgbw_ms_to_slope: 'canonical', // lp_time_ms table
    tile_display_rgbw_set_autonomous: 'canonical',
    tile_display_rgbw_set_aeu: 'hallucinated', // keyframes can't cross the flat ABI
    tile_display_rgbw_set_animation: 'inferred', // only the channel is captured
    tile_display_rgbw_set_exp_dimming: 'canonical',
    tile_display_rgbw_set_phase_align: 'canonical',
    tile_display_rgbw_update: 'inferred', // config latching not modeled
  ...
  deriveState(state, { t }) {
    ...
    if (state.fault_inject === 1 && state.fault_short_mask !== 0x0f) update.fault_short_mask = 0x0f;
    ...
    if (state.helper_running === 1) {
      ...
      let on = 0; // fraction of the peak lit now
      let done = false;
      if (state.helper_kind === HELPER_PULSE) {
        done = el >= state.helper_ms;
        on = done ? 0 : 1;
      } else if (state.helper_kind === HELPER_BREATHE) {
        const p = Math.max(64, state.breathe_period_ms); // step_ms clamps to ≥ 1 ms
        done = el >= p;
  ...
  indicators(state, ctx) {
    ...
    const lit = (ch: Ch) => (alive ? level(state, ch) : 0);
    const ind: TileIndicator[] = [
