...
var outputActive = (s) => s.output_on === 1 && isPlayMode(s.mode);
...
var scale = (sample, pct) => Math.trunc(sample * Math.min(100, pct) * REF_MAX / (100 * SAMPLE_FS));
...
var outFsMv = (s) => s.output_range === 1 ? 15588 : 111600;
function senseRaw(s) {
...
function play(s, label, peak, freq, ms, hold, stopAfter) {
  return {
...
var CLICK_SAMPLES = 19;
var CLICK_MS = CLICK_SAMPLES / 8;
...
var sim = {
  ...
  defaultState: {
    ...
    supply_mv: 3700,
    load_nf: 260,
    // init tunes PARCAP for a 260 nF piezo (tile_drive_p_init_at)
    mode: MODE_IDLE,
    output_on: 0,
    stop_after_drain: 0,
    sleeping: 0,
  ...
  hostCalls: {
    ...
    tile_drive_p_play_pulse_train: ({ state, args }) => {
      ...
      return {
        nextState: play(
          ...
          clickPeak(pct),
          count > 1 ? Math.round(1e3 / (CLICK_MS + gap)) : 0,
    ...
    tile_drive_p_read_sense_samples: ({ state, caps }) => {
      ...
      const sensing = { ...state, ...setMode(state, MODE_SENSE_FINE) };
      return {
        out: { buf: new Array(n).fill(senseRaw(sensing)) },
        nextState: {
          ...setMode(state, MODE_SENSE_FINE),
          return_reg: REG_SENSE_VAL,
  ...
  provenance: {
    ...
    // driver's 0 V-framed half-sine, 100 % = 1743
    tile_drive_p_play_sine: "canonical",
    // driver's Q12 phase, ends at 0 V
    ...
    // reset + full config restore; timing not modeled
    tile_drive_p_sleep: "inferred",
    tile_drive_p_wake: "inferred",
    // outcome only; RET=1 register loss is restored by the driver
    ...
    tile_drive_p_play_samples: "inferred",
    // envelope = peak |sample|, clamped ±1743
    tile_drive_p_read_sense_samples: "inferred",
    // a constant press level, no waveform
    power: "inferred"
  ...
  deriveState: (state, { t }) => {
    ...
    if (state.playing_until_ms !== 0 && t >= state.playing_until_ms) {
      ...
      const tout = state.auto_sleep && state.output_on === 1 && (state.mode === MODE_PLAY_FIFO || state.mode === MODE_PLAY_DIRECT);
      return {
        playing_until_ms: 0,
        currently_playing: "",
        freq_hz: 0,
    ...
    return {
      ...
      rails: [
        ...
        },
        {
          ...
          pads: ["7", "8"],
          note: "boosted differential piezo drive (\xB195 V at code 1743)"
        }
      ]
    };
  }
