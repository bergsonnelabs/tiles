...
var womThrMg = (mg) => Math.min(255, Math.trunc((Math.trunc(mg) * 256 + 500) / 1e3)) * 1e3 / 256;
var ATAN_LUT_CDEG = [
  0,
  358,
  712,
...
function atan2Cdeg(y, x) {
  ...
  const lut = (num, den) => {
    ...
    return ATAN_LUT_CDEG[idx] + Math.trunc((ATAN_LUT_CDEG[idx + 1] - ATAN_LUT_CDEG[idx]) * rem / den);
  };
  let r = ay <= ax ? lut(ay, ax) : 9e3 - lut(ax, ay);
  if (x < 0) r = 18e3 - r;
  if (y < 0) r = -r;
  return r;
}
function isqrtDriver(r) {
  if (r <= 0) return r;
  let g = r >> 8 || 1;
  for (let i = 0; i < 16; i++) {
...
function tiltCdeg(a, axis) {
  ...
  return Math.max(-18e3, Math.min(18e3, c));
}
...
var sim = {
  ...
  hostCalls: {
    ...
    tile_sense_i_6p6_fifo_count: ({ state }) => ({
      scalar: state.fifo_packets & 65535
    }),
    tile_sense_i_6p6_fifo_lost_count: ({ state }) => ({
      scalar: state.fifo_lost & 65535
    }),
    ...
    tile_sense_i_6p6_get_step_count: ({ state }) => ({
      scalar: state.step_count & 65535
    }),
    tile_sense_i_6p6_get_step_cadence: ({ state }) => ({
      scalar: state.step_cadence & 255
    }),
    tile_sense_i_6p6_get_activity: ({ state }) => ({
      scalar: state.activity & 3
    }),
    // ── APEX: tilt / tap ──
  ...
  provenance: {
    ...
    // records (FIFO_COUNT_REC = 1)
    tile_sense_i_6p6_wom_disable: "canonical",
    // SMD_MODE = 00 (untested on hardware)
    ...
    tile_sense_i_6p6_read_tilt_centi_degrees_flat: "canonical",
    // driver's integer atan2
